...
import { DollarSign, TrendingUp, Users, Award, Check, ChevronDown, ChevronUp, ArrowRight } from 'lucide-react';
    ...
    <div className="space-y-16">
      ...
      <div className="relative rounded-3xl overflow-hidden">
        ...
        <div className="relative px-8 py-16 md:py-24 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#e91e8c]/20 rounded-full text-[#e91e8c] text-sm font-medium mb-6">
            <Sparkles className="w-4 h-4" />
            now accepting new affiliates
          </div>

          <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">
            join the <span className="text-[#e91e8c]">ourdream.ai</span><br />
            affiliate program
          </h1>

          <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto">
            become an ourdream affiliate and earn{' '}
            <span className="text-[#e91e8c] font-semibold">40% revshare</span> or{' '}
            <span className="text-[#e91e8c] font-semibold">up to $50 cpa</span> on every referred customer.
          </p>

          <div className="flex flex-wrap justify-center gap-4">
            <button className="px-8 py-4 bg-gradient-to-r from-[#e91e8c] to-[#ff1493] hover:opacity-90 rounded-full text-white font-semibold flex items-center gap-2 transition-opacity">
              apply now
              <ArrowRight className="w-5 h-5" />
            </button>
            <button className="px-8 py-4 bg-[#1a1a1a] hover:bg-[#2a2a2a] rounded-full text-white font-semibold transition-colors">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="text-center">
            <p className="text-3xl md:text-4xl font-bold text-[#e91e8c] mb-2">{stat.value}</p>
      ...
      <div className="text-center max-w-3xl mx-auto">
        <p className="text-xl md:text-2xl text-[#e91e8c] italic mb-4">
      ...
      <div>
        <h2 className="text-3xl font-bold text-white text-center mb-4">
          join the <span className="text-[#e91e8c]">dream team</span>
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {features.map((feature) => {
            ...
            return (
              <div key={feature.title} className="bg-[#1a1a1a] rounded-2xl p-6">
                <div className="w-12 h-12 rounded-full bg-[#e91e8c]/20 flex items-center justify-center mb-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {commissionModels.map((model) => (
            <div key={model.title} className="bg-[#1a1a1a] rounded-2xl p-8">
              <h3 className="text-2xl font-bold text-white mb-2">{model.title}</h3>
              <p className="text-gray-400 mb-4">{model.description}</p>
              <p className="text-4xl font-bold text-[#e91e8c] mb-6">{model.rate}</p>
      ...
      <div className="text-center">
        ...
        <button className="px-8 py-4 bg-gradient-to-r from-[#e91e8c] to-[#ff1493] hover:opacity-90 rounded-full text-white font-semibold transition-opacity">
          apply now
        </button>
...
// Add Sparkles icon since it's used but not imported
